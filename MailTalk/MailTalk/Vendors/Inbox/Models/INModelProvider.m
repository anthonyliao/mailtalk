...
//	[_fetchOperation setResponseSerializer:serializer];
    ...
    [[INAPIManager shared].MT GET:_namespaceID
                         function:path
                       parameters:params
                          success:^(id result, NSError *error) {
                              INModelResponseSerializer * serializer = [[INModelResponseSerializer alloc] initWithModelClass: _modelClass];
                              [serializer setModelsCurrentlyMatching: [NSArray arrayWithArray: self.items]];
                              id threads = [serializer responseObjectForResponse:nil data:result error:&error];
                              
                              NSLog(@"GET %@ (%@) RETRIEVED %lu %@s", path, [params description], (unsigned long)[threads count], NSStringFromClass(_modelClass));
